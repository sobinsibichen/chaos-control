    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {data.liveCraving?.insightText ?? `Danger window detected around ${dangerousWindow.label}. Probability elevated for the next 30 minutes.`}
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <GlassCard className="border border-foreground/10">
          <CircularMeter value={data.liveCraving?.cravingProbability ?? dangerousWindow.intensity} label="RISK" sub="next craving" size={130} color="oklch(0.65 0.18 30)" />
        </GlassCard>
        <GlassCard className="border border-foreground/10">
          <CircularMeter value={data.liveCraving?.intensityScore ?? Math.min(100, (data.dashboard?.dailyStatus.regretLevel ?? 0) + 18)} label="STRESS" sub="trigger load" size={130} color="oklch(0.62 0.16 15)" />
        </GlassCard>
      </div>
      ...
      <GlassCard className="border border-foreground/10">
        ...
        <div className="h-60">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.hourlyCraving}>
      ...
      <GlassCard className="border border-foreground/10">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="rounded-2xl bg-background p-4">
            ...
            <div className="mt-2 text-lg font-semibold text-foreground">{data.liveCraving?.triggerPrediction.primary ?? "Routine loop"}</div>
          </div>
        </div>
      </GlassCard>
