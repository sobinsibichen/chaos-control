...
export function MentalStabilityChallenge({ open, onClose, onResult }: MentalStabilityChallengeProps) {
  ...
  const failChallenge = () => {
    ...
    onResult?.({ passed: false, challengeText: paragraph, requiredAccuracy: 100, attempts: attempts + 1 });
    setTyped("");
  ...
  const handleChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (nextValue === paragraph) {
      setCompleted(true);
      appStore.unlockApps();
      onResult?.({ passed: true, challengeText: paragraph, requiredAccuracy: 100, attempts: attempts + 1 });
    }
  };
  ...
  const charStates = useMemo(
    () => paragraph.split("").map((character, index) => ({ character, state: getCharacterState(paragraph, typed, index) })),
    [paragraph, typed],
  );
          ...
          >
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">One mistake fails the attempt. The paragraph stays visible while you type.</p>
              </div>
            ...
            <div className="mb-2 flex items-center justify-between text-[11px] font-medium uppercase tracking-[0.15em] text-muted-foreground">
              <span>{completedCount} / {paragraph.length} characters completed</span>
              <span>{attempts} failed attempts</span>
            </div>
            ...
            <div className="relative flex-1 overflow-hidden rounded-[1.75rem] border border-foreground/10 bg-card p-4">
              <div className="max-h-[42vh] overflow-y-auto pr-2 text-[15px] leading-7">
                {charStates.map((entry, index) => {
                  ...
                  const isCursor = index === typed.length && !failed && !completed;
                  ...
                  return (
                    ...
                    >
                      {entry.character}
                      {isCursor ? <span className="absolute -bottom-1 left-0 h-5 w-[1px] animate-pulse bg-foreground" /> : null}
                    </span>
                  );
                })}
              </div>
            ...
            <AnimatePresence mode="wait">
              {failed ? (
                ...
                >
                  Mistake detected. Resetting to the beginning.
                </motion.div>
              ) : completed ? (
                <motion.div
                  key="completed"
